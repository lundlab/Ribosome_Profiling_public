...
9) Exon spanning alignments of RPF and total RNA were converted to bigwig files with deeptools (v3.3.1). Replicates for each condition were CPM normalised individually. The Replicates from each condition were merged by calculating their mean coverage per base. For RPF bigwig files, only exon spanning 28-31nt reads were included. For total RNA bigwig files, no filtering for read length applied. Reads mapping to the canonical transcripts selected to represent each protein-coding are included in the bigwig files.
